...
Buffer::Buffer(Run starting_run) {
  run_ = starting_run;
  records_ = std::vector<Record>();
}

void Buffer::init(Belt &belt) { load_next_records(belt); }

std::optional<Record> Buffer::get_record(Belt &belt) {
...
bool Buffer::load_next_records(Belt &belt) {
...
void Buffer::append_out_buffer(Record &&record, Belt &belt) {
...
// empties the buffer
void Buffer::write_buffer(Belt &belt) {
  if (records_.size() > 0) {
    belt.append_to_file(records_);
    records_.clear();
    return;
...
void Buffer::clear() { records_.clear(); }
